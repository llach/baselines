...
class VAEModel(object):
    ...
    def __init__(self, k, policy, ob_space, ac_space, nbatch_act, nbatch_train, savepath, env, vae_params, rl_coef,
        ...
        with tf.variable_scope('ppo_buf_model', reuse=tf.AUTO_REUSE):
        ...
        # Total loss
        ppo_loss = pg_coef * pg_loss - entropy * ent_coef + vf_loss * vf_coef

        joint_loss = rl_coef * ppo_loss + self.vae.vae_loss
        ...
        def _filter_gv(fi):
            return [gv for gv in grads_and_var if fi in gv[1].name]
    ...
    def step(self, obs, **extra_feed):
        ...
        return am.sess.run([am.action, am.vf, am.state, am.neglogp, self.vae.re_loss], feed_dict=feed_dict)
    ...
    def save(self, savepath):
        print('saving model ... ')
        save_variables(f'{savepath}/model', tf.trainable_variables('ppo_buf_model'), sess=self.sess)
        self.vae.save()

    def load(self, loadpath):
        print('loading model ...')
        load_variables(f'{loadpath}/model', tf.trainable_variables('ppo_buf_model'), sess=self.sess)
        self.vae.load()

    def train_ppo(self, lr, cliprange, obs, returns, actions, values, neglogpacs):
        return self.train(lr, cliprange, obs, returns, actions, values, neglogpacs, typ='ppo')

    def train_vae(self, lr, cliprange, obs, returns, actions, values, neglogpacs):
        return self.train(lr, cliprange, obs, returns, actions, values, neglogpacs, typ='vae')

    def train_full(self, lr, cliprange, obs, returns, actions, values, neglogpacs):
        return self.train(lr, cliprange, obs, returns, actions, values, neglogpacs, typ='full')
    ...
    def train(self, lr, cliprange, obs, returns, actions, values, neglogpacs, typ=''):
        ...
        # chose approriate training op
        if typ == 'ppo':
            train_op = self._ppo_train_op
        elif typ == 'vae':
            train_op = self._vae_train_op
        elif typ == 'full':
            train_op = self._full_train_op
        else:
            print(f'train type {typ} unknown!')
            exit(0)
        ...
        res = self.sess.run(
            self.stats_list + [self.vae.re_loss, self.vae.kl_loss, train_op],
            td_map
        )[:-1]
